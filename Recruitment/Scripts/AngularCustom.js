    ...
    $scope.save = function () {
       ...
       $scope.Candidate.sur_name != "" && $scope.Candidate.position != "" && $scope.files.length > 0) {
            ...
            $http({
                ...
                $scope.candidateData.push(response.data);
                //$scope.clear();
                alert("Candidate Added Successfully !!!");
                Quixxi.q.push(['SendEvent', {
                    "event_category": "Candidate",
                    "event_value": "Add Form",
                    "event_detail": "Event when click button Save on Add Candidate Form",
                    "event_object":
                        {
                            "id": $scope.Candidate.id,
                            "name": $scope.Candidate.name,
                            "sur_name": $scope.Candidate.sur_name,
    ...
    $scope.update = function () {
       ...
       $scope.Candidate.sur_name != "" && $scope.Candidate.position != "") {
            $http({
                ...
                Quixxi.q.push(['SendEvent', {
                    "event_category": "Candidate",
                    "event_value": "Edit Form",
                    "event_detail": "Event when click button Update on Edit Candidate Form",
                    "event_object":
                        {
                            "id": $scope.Candidate.id,
                            "name": $scope.Candidate.name,
